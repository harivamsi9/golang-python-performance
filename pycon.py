...
# Asyncio approach
async def asyncio_requests(urls):
    tasks = []
    for url in urls:
        tasks.append(asyncio.to_thread(make_request, url))
...
def threading_requests(urls):
    ...
    results = []
    for url in urls:
        thread = threading.Thread(target=lambda: results.append(worker(url)))
...
if __name__ == "__main__":
    # Example URLs to simulate requests
    # urls = ['https://www.google.com'] * 100  # Simulating 100 requests
    urls = [
    ...
    # urls = urls[:25]  # Limit to 10 URLs for testing



    # # Measure performance of sequential approach
    # start_time = time.time()
    # sequential_requests(urls)
    # print(f"Sequential time: {time.time() - start_time} seconds")


    # # Measure performance of asyncio
    # start_time = time.time()
    # asyncio.run(asyncio_requests(urls))
    # print(f"Asyncio time: {time.time() - start_time} seconds")

    # # Measure performance of threading
    # start_time = time.time()
    # threading_requests(urls)
    # print(f"Threading time: {time.time() - start_time} seconds")

    # Measure performance of concurrent.futures
    start_time = time.time()
    futures_requests(urls)
    print(f"Futures time: {time.time() - start_time} seconds")
